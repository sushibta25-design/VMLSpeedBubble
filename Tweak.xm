...
static void VMLCreateOrRefreshSingleOverlay(void) {
    ...
    if (!gCarPlayOverlayWindow) {
        ...
        VMLLog(
            @"*** CLEAN CARPLAY OVERLAY CREATED V14.1.2 scene=%@ frame=%@ ***",
...
%ctor {
    @autoreleasepool {
        VMLLog(@"========================================");
        VMLLog(@"VML SPEED BUBBLE V14.1.2 TRACE BUILD FIX");
        ...
        if (VMLIsSpringBoard()) {
            ...
            VMLLog(@"V14.1.2 SPRINGBOARD ACTIVE");
            return;
        }
